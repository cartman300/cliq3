...
qboolean BotIsDead(bot_state_t *bs) {
	return toqbool(bs->cur_ps.pm_type == PM_DEAD);
}
...
qboolean BotIntermission(bot_state_t *bs) {
	...
	return toqbool(bs->cur_ps.pm_type == PM_FREEZE || bs->cur_ps.pm_type == PM_INTERMISSION);
}
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
	...
	feet[2] -= 23;
	return toqbool(trap_AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME));
}
