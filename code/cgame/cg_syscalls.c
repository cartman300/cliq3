...
typedef intptr_t (QDECL *SYSCALLFUNC)(intptr_t, ...);

//static intptr_t (QDECL *syscall)( intptr_t arg, ... ) = (intptr_t (QDECL *)( intptr_t, ...))-1;
static SYSCALLFUNC syscall = (SYSCALLFUNC)0;

Q_EXPORT void dllEntry( /*intptr_t (QDECL  *syscallptr)( intptr_t arg,... )*/SYSCALLFUNC syscallptr ) {
	syscall = syscallptr;
}
...
qboolean	trap_GetSnapshot( int snapshotNumber, snapshot_t *snapshot ) {
	return toqbool(syscall( CG_GETSNAPSHOT, snapshotNumber, snapshot ));
}

qboolean	trap_GetServerCommand( int serverCommandNumber ) {
	return toqbool(syscall( CG_GETSERVERCOMMAND, serverCommandNumber ));
}
...
qboolean	trap_GetUserCmd( int cmdNumber, usercmd_t *ucmd ) {
	return toqbool(syscall( CG_GETUSERCMD, cmdNumber, ucmd ));
}
...
qboolean trap_Key_IsDown( int keynum ) {
	return toqbool(syscall( CG_KEY_ISDOWN, keynum ));
}
...
qboolean trap_GetEntityToken( char *buffer, int bufferSize ) {
	return toqbool(syscall( CG_GET_ENTITY_TOKEN, buffer, bufferSize ));
}

qboolean trap_R_inPVS( const vec3_t p1, const vec3_t p2 ) {
	return toqbool(syscall( CG_R_INPVS, p1, p2 ));
}
